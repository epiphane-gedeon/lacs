    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            if not parent_existant:
                # Générer le code parent en premier
                ...
                utilisateur_parent = Utilisateur(
                    ...
                    numero_telephone=form.telephone_parent.data,
                    email=f"parent_{code_parent.lower().replace('-', '_')}@temp.lacs",  # Email basé sur le code parent
            ...
            # 3. Générer le matricule en premier
            matricule = generer_matricule()

            # 4. Créer l'utilisateur élève avec email basé sur le matricule
            utilisateur_eleve = Utilisateur(
                ...
                if form.email.data
                else f"eleve_{matricule.lower().replace('-', '_')}@temp.lacs",
                mot_de_passe=generate_password_hash(
                    "temp123"
                ),  # Mot de passe temporaire
                admin=False,
            ...
            # 5. Créer l'élève avec le matricule généré
            eleve = Eleve(
                matricule=matricule,
...
def liste_inscriptions():
    ...
    # Ordonner par matricule de l'élève (ordre croissant pour avoir LACS-25-A001, A002, etc.)
    inscriptions = Inscription.query.join(Eleve).order_by(Eleve.matricule.asc()).all()
